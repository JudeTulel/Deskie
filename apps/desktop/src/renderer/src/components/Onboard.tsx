...
export default function Onboard() {
  ...
  const handleContinue = async () => {
    if (step < TOTAL_STEPS - 1) {
      ...
      // Final step — persist to SQLite via IPC
      setSaving(true)
      setError('')
      try {
        ...
        // userStore.hydrated will now be true with userDetails set → App gate opens
      } catch (err: any) {
  ...
  return (
    <section className="h-screen overflow-hidden bg-transparent flex flex-col items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white/80 dark:bg-white/5 backdrop-blur-sm rounded-3xl p-8 shadow-sm border border-black/8 dark:border-white/8">
        ...
        {/* Error message */}
        {error && (
          <div className="mb-4 px-4 py-2.5 bg-red-900/20 border border-red-500/30 rounded-lg text-xs text-red-400 font-medium text-center">
            {error}
          </div>
        )}
