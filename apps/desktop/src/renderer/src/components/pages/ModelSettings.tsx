...
function methodLabel(src: string): string { return src.startsWith('pear://') ? 'P2P (Hyperdrive)' : 'HTTP' }
...
const DownloadItem: React.FC<{
  ...
  const statusColor: Record<DownloadEntry['status'], string> = {
    queued: 'text-amber-400', downloading: 'text-sky-400', completed: 'text-emerald-400',
    cancelled: 'text-slate-500', error: 'text-red-400',
  }
  const barColor: Record<DownloadEntry['status'], string> = {
    queued: 'from-amber-500 to-yellow-400', downloading: 'from-sky-500 to-blue-400',
    completed: 'from-emerald-500 to-green-400', cancelled: 'from-slate-600 to-slate-500',
    error: 'from-red-600 to-red-500',
  }

  return (
    <div className="group relative rounded-xl border border-slate-700/60 bg-slate-800/30 p-4 transition-all hover:border-slate-600/80" style={{ backdropFilter: 'blur(8px)' }}>
      ...
      {(isActive || item.status === 'completed') && (
        <div className="mb-3">
          <div className="h-1.5 rounded-full bg-slate-700/60 overflow-hidden">
            <div className={`h-full rounded-full bg-gradient-to-r ${barColor[item.status]} transition-all duration-300`} style={{ width: `${item.percentage}%` }} />
          </div>
          <div className="flex justify-between mt-1.5 text-[11px] text-slate-500">
            <span>{item.total > 0 ? `${formatBytes(item.downloaded)} / ${formatBytes(item.total)}` : formatBytes(item.downloaded)}</span>
      ...
      {item.status === 'error' && item.error && (
        <p className="text-[11px] text-red-400 mb-3 font-mono bg-red-900/10 rounded p-2 border border-red-900/30">{item.error}</p>
      )}
        ...
        <div className="flex gap-1.5">
          {item.status === 'completed' && item.assetId && (
            <button
              ...
              className={`flex items-center gap-1 rounded-md border px-2.5 py-1 text-[11px] transition-all ${isActiveModel ? 'border-emerald-500/30 bg-emerald-900/10 text-emerald-400 cursor-default' : 'border-blue-500/40 text-blue-300 hover:bg-blue-900/20'}`}
            >
              <IconCheck /> {isActiveModel ? 'Active' : 'Set active'}
            </button>
          )}
          {isActive && (
            <button id={`cancel-download-${item.id}`} onClick={() => onCancel(item.id)}
          ...
          {!isActive && (
            <button id={`remove-download-${item.id}`} onClick={() => onRemove(item.id)}
...
const ModelSettings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = window.qvacAPI.onDownloadProgress((data) => {
      setDownloads((prev) =>
        prev.map((d) => d.id === data.downloadId
          ? { ...d, percentage: data.percentage, downloaded: data.downloaded, total: data.total, downloadKey: data.downloadKey, status: 'downloading' as const }
          : d
        )
      )
    })
  ...
  const startDownload = useCallback(async (assetSrc: string, name: string) => {
    const id = uid()
    const entry: DownloadEntry = { id, name, assetSrc, percentage: 0, downloaded: 0, total: 0, downloadKey: '', status: 'queued', startedAt: Date.now() }
    ...
    try {
      ...
      if (!result.cancelled && result.assetId) {
        await saveModel({ name, assetSrc, assetId: result.assetId, localPath: result.localPath, type: name.toLowerCase().includes('whisper') ? 'Speech' : 'LLM', status: 'completed' })
      }
    } catch (err: any) {
      setDownloads((prev) => prev.map((d) => (d.id === id ? { ...d, status: 'error', error: err?.message ?? String(err) } : d)))
    }
  }, [saveModel])

  const handleCatalogueDownload = (model: CatalogModel) => { startDownload(model.src, model.name) }
  const handleCustomDownload = () => {
    ...
    setCustomSrc(''); setCustomName('')
  }
  const handleCancel = async (id: string) => {
    await window.qvacAPI.cancelDownload(id)
    setDownloads((prev) => prev.map((d) => (d.id === id ? { ...d, status: 'cancelled' } : d)))
  }
  const handleRemove = (id: string) => { setDownloads((prev) => prev.filter((d) => d.id !== id)) }
  const handleSetActiveDownload = async (item: DownloadEntry) => {
    await saveModel({ name: item.name, assetSrc: item.assetSrc, assetId: item.assetId, localPath: item.localPath, type: item.name.toLowerCase().includes('whisper') ? 'Speech' : 'LLM', status: 'completed', isActive: true })
  }
  const handleSetActiveCatalog = async (model: CatalogModel) => {
    await saveModel({ name: model.name, assetSrc: model.src, type: model.type === 'LLM' || model.type === 'Speech' ? model.type : 'Other', status: 'available', isActive: true })
  }
    ...
    <div className="space-y-10">
      ...
      <div className="flex items-center gap-2">
        {(['catalogue', 'custom'] as const).map((m) => (
          <button key={m} id={`model-mode-${m}`} onClick={() => setDownloadMode(m)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${downloadMode === m ? 'bg-white text-slate-900' : 'bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700'}`}>
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-1.5 rounded-lg border border-slate-700/60 bg-slate-800/40 p-1">
              {(['all', 'LLM', 'Speech'] as const).map((f) => (
                <button key={f} id={`filter-${f}`} onClick={() => setFilterType(f)}
                  className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${filterType === f ? 'bg-slate-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}>
                  {f === 'all' ? 'All' : f}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredCatalog.map((model) => {
              const alreadyQueued = downloads.some((d) => d.assetSrc === model.src && (d.status === 'downloading' || d.status === 'queued'))
              const completed = downloads.some((d) => d.assetSrc === model.src && d.status === 'completed')
              ...
              return (
                <div key={model.name} className="relative group rounded-xl border border-slate-700/60 bg-slate-800/30 p-5 flex flex-col gap-3 hover:border-slate-600 transition-all" style={{ backdropFilter: 'blur(8px)' }}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex flex-wrap gap-1.5">
                      <span className="text-[10px] font-semibold uppercase tracking-wide bg-slate-700/60 text-slate-300 rounded px-1.5 py-0.5">{model.type}</span>
                      ...
                      {model.badge && (
                        <span className="text-[10px] font-semibold bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-amber-300 border border-amber-500/30 rounded px-1.5 py-0.5">{model.badge}</span>
                      )}
                    </div>
                    {completed && <span className="flex items-center gap-1 text-[10px] text-emerald-400 font-medium"><IconCheck /> Done</span>}
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-2 border-t border-slate-700/40">
                    ...
                    {canSetActive && (
                      <button
                        ...
                        disabled={isActiveModel}
                        className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${isActiveModel ? 'bg-emerald-900/20 text-emerald-400 border border-emerald-500/30 cursor-default' : 'border border-blue-500/40 text-blue-300 hover:bg-blue-900/20'}`}>
                        <IconCheck /> {isActiveModel ? 'Active' : 'Set active'}
                      </button>
                    )}
                    <button
                      ...
                      className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${completed ? 'bg-emerald-900/20 text-emerald-400 border border-emerald-500/30 cursor-default' : alreadyQueued ? 'bg-slate-700/60 text-slate-500 cursor-not-allowed border border-slate-600/40' : 'bg-gradient-to-r from-blue-600 to-blue-500 text-white hover:from-blue-500 hover:to-blue-400 shadow-lg shadow-blue-900/30'}`}>
                      {completed ? <><IconCheck /> Downloaded</> : alreadyQueued ? 'In queue…' : <><IconDownload /> Download</>}
                    </button>
                  </div>
                </div>
              )
      ...
      {downloadMode === 'custom' && (
        <section>
          ...
          <p className="text-sm text-slate-400 mb-6">
            Paste an HTTP URL or a <code className="text-violet-400 font-mono text-xs bg-violet-900/20 px-1 rounded">pear://&lt;key&gt;/path</code> Hyperdrive address
          </p>
          <div className="rounded-xl border border-slate-700/60 bg-slate-800/30 p-6 space-y-5" style={{ backdropFilter: 'blur(8px)' }}>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-2">Model source <span className="text-slate-600">(HTTP or pear://)</span></label>
              <div className="relative flex-1">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">
                  {customSrc.startsWith('pear://') ? <IconP2P /> : <IconGlobe />}
                </span>
                <input id="custom-model-src" type="text" value={customSrc} onChange={(e) => setCustomSrc(e.target.value)}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-2">Display name <span className="text-slate-600">(optional)</span></label>
              <input id="custom-model-name" type="text" value={customName} onChange={(e) => setCustomName(e.target.value)}
            ...
            {customSrc.trim() && (
              <div className={`flex items-center gap-2 text-xs px-3 py-2 rounded-lg border ${methodColor(customSrc)}`}>
                {customSrc.startsWith('pear://') ? <IconP2P /> : <IconGlobe />}
                <span className="font-medium">{customSrc.startsWith('pear://') ? 'P2P Hyperdrive — model will be seeded by connected peers' : 'HTTP download — fetching directly from remote server'}</span>
              </div>
            )}
            <button id="custom-download-btn" onClick={handleCustomDownload} disabled={!customSrc.trim()}
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-0.5">{activeCount > 0 ? `${activeCount} active` : 'All downloads finished'}</p>
            </div>
            {activeCount === 0 && (
              <button id="clear-all-downloads" onClick={() => setDownloads([])}
          ...
          <div className="space-y-3">
            {downloads.map((d) => (
              <DownloadItem key={d.id} item={d} activeModel={activeModel} onCancel={handleCancel} onRemove={handleRemove} onSetActive={handleSetActiveDownload} />
            ))}
          </div>
        </section>
      )}
